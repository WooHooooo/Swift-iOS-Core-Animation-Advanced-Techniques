...
class ReboundBall_NSTimer: UIViewController {
    ...
    func animate(){
        ...
        //timer.invalidate()
        timer = Timer.scheduledTimer(timeInterval: 1/60.0, target: self, selector: #selector(step(step:)), userInfo: nil, repeats: true)
    }
    
    @objc func step(step: Timer){
